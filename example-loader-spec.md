...
|---|---|---|---|---|---|---|
| `handlebars` | handlebarsjs.com markdown source (VitePress) | prose examples (template + context) | **computed** by real Handlebars at vendor time | **expected-difference** | FullBars | MIT © Yehuda Katz |
| `mustache` | `github.com/mustache/spec` YAML | `template` + `data` + `expected` (+ `partials`) | **authoritative** (in file) | **conformance-failure** | MinBars | MIT |
...
> Action item: the exact handlebarsjs.com markdown source repo/path was not pinned during review (confirmed VitePress + MIT; repo path TBD). The `mustache/spec` path is known (`specs/*.yml`).
...
`expected` is the only place the two providers diverge in code: the Handlebars provider shells to a pinned real-Handlebars at vendor time; the Mustache provider reads the YAML field. Everything downstream (fixture format, TUI, diff, playground load) is shared.
...
```jsonc
{
  ...
  "unsupported": ["inline-partial"],            // FullBars gap (see §7)
...
2. **expected**: Handlebars → run pinned real-Handlebars (dev-only dep) to compute output; Mustache → read the YAML `expected`.
...
The pipeline is **deterministic**: same commit ⇒ byte-identical fixtures. Re-running is the only "refresh"; there is no runtime fetch.
...
- **FullBars vs Handlebars `/examples/`:** the completeness patch (`rawbars-maxbars-spec.md` §5) covered `@../`, `@root`, recursive partials — but **not** Handlebars's `{{#*inline}}` inline partials or `{{#> …}}` / `{{> @partial-block}}` partial-blocks. The cited `partials/inline` example uses exactly those, so it will tag `uses-unsupported-feature` on day one. This is a genuine finding: either **fill the gap** (add inline-partials/partial-blocks to FullBars — Handlebars's own inheritance mechanism, distinct from MinBars's `{{<}}`/`{{$}}`) or **flag** affected examples. Tracked as a decision (§10).
...
- **Deterministic baking.** Expected output is computed once at a pinned commit (Handlebars) or read verbatim (Mustache); `verify` re-checks determinism.
...
- **E1 — Vendor pipeline + `mustache` corpus.** `discover/parse/expected/features`; vendor the spec YAML at a pinned commit; `check`/`verify` headless.
- **E2 — Playground load + diff.** Wire the engine; expected-vs-actual with `divergenceMeaning` framing; auto-dialect-select.

### Handlebars provider
- **E3 — `handlebars` provider + expected-baking.** Locate the markdown source repo (§2 action item); parse examples; compute expected via pinned real-Handlebars; feature-tag against FullBars; record the divergence catalogue.

### TUI
- **E4 — TUI shell.** Ink app: provider switch, tree with status glyphs, filter, detail/diff pane, vendor/update/verify actions, attribution view, help overlay; headless flags wired to the same actions.
...
1. **FullBars inline-partials/partial-blocks** (§7) — fill the gap (add `{{#*inline}}` + `{{> @partial-block}}` to FullBars) or flag affected Handlebars examples. Filling makes FullBars more complete; flagging is cheaper.
...
5. **Divergence catalogue churn** — when a new Handlebars divergence appears at `verify`, is it a hard CI fail (must be reviewed/recorded) or a warning? Hard-fail is safer; confirm.
