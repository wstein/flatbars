...
(one lesson per surface). MinBars, RawBars, and FullBars have no per-surface
lesson — their dedicated references supersede it; only MaxBars is generated by
`src/pages/[surface].astro` today.
...
- **One source per example, gate-validated:** the Mustache reference's runnable
  ...
  the FullBars reference's in `src/fullbars.mjs`, the per-surface lessons in
