...
corpus =
  ...
  , Tuple "escaped path" "{{{ esc_html (lookup this \"a\") }}}"
  , Tuple "raw path" "{{{ lookup this \"a\" }}}"
  , Tuple "and" "{{{ esc_html (and (lookup this \"done\") (lookup this \"done\")) }}}"
  , Tuple "gt" "{{{ esc_html (gt (lookup this \"b\") 21) }}}"
  ...
  , Tuple "unary filter esc_json" "{{{ esc_json (lookup this \"o\") }}}"
  ...
  , Tuple "each loop vars"
      "{{#each (lookup this \"xs\")}}[{{{index1}}}/{{{ esc_html this }}}]{{/each}}"
  , Tuple "each with infix cond"
      "{{#each (lookup this \"xs\")}}{{#if (gt index0 0)}}, {{/if}}{{{ esc_html this }}}{{/each}}"
  , Tuple "if elif else"
