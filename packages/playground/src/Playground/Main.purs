...
examples =
  ...
  , { id: "truthiness"
    ...
    , template:
        "<h3>Truthiness — FlatBars vs Handlebars</h3>\n<table border=\"1\" cellpadding=\"5\">\n<tr><th>value</th><th>FlatBars</th><th>Handlebars</th></tr>\n{{#each (lookup this \"cases\")}}<tr><td>{{{esc_html (lookup this \"label\")}}}</td><td>{{#if (lookup this \"v\")}}truthy{{else}}falsy{{/if}}</td><td>{{{esc_html (lookup this \"hb\")}}}</td></tr>\n{{/each}}<tr><td>0 + includeZero</td><td>{{#if (lookup this \"zero\") (dict \"includeZero\" true)}}truthy{{else}}falsy{{/if}}</td><td>truthy</td></tr>\n</table>\n<p>FlatBars matches Handlebars: <b>0</b> is falsy by default, truthy with <code>(dict \"includeZero\" true)</code>. Falsy = false, null, 0, \"\", [], and a trusted empty string.</p>"
    , dataText:
