...
tokenizeTemplate src = map finalize (go 0 0 [] Nil false)
  ...
  openerLiteralAt i
    ...
    | matchAt cs i "{{~#>" = Just "{{~#>"
    | matchAt cs i "{{#>" = Just "{{#>" -- block partial (Handlebars `{{#>name}}`)
    | matchAt cs i "{{~#*" = Just "{{~#*"
    | matchAt cs i "{{#*" = Just "{{#*" -- inline-partial decorator (Handlebars `{{#*inline …}}`)
    | matchAt cs i "{{~/" = Just "{{~/"
    | matchAt cs i "{{/" = Just "{{/"
    | matchAt cs i "{{~&" = Just "{{~&"
  ...
  readTag i
    ...
    | matchAt cs i "{{#>" = readBlockOpen i "{{#>" PartialBlock "}}"
    | matchAt cs i "{{~#*" = readBlockOpen i "{{~#*" InlineDecorator "}}"
    | matchAt cs i "{{#*" = readBlockOpen i "{{#*" InlineDecorator "}}"
